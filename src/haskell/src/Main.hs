...
parseString :: Parser LispVal; parseString = do
  char '"'; x <- many (noneOf "\""); char '"'; return $ String x

parseAtom :: Parser LispVal; parseAtom = do
  first <- letter <|> symbol
...
parseExpr :: Parser LispVal; parseExpr = parseAtom <|> parseString <|> parseNumber <|> parseQuoted <|> do
  char '('; x <- try parseList <|> parseDottedList; char ')'; return x
...
boolBinop unpacker op args = if length args /= 2 then throwError $ NumArgs 2 args else do
  left <- unpacker $ args !! 0; right <- unpacker $ args !! 1; return $ Bool $ left `op` right
...
primitives = [("+", numericBinop (+)),("-", numericBinop (-)), ("*", numericBinop (*)), ("/", numericBinop div), 
  ("mod", numericBinop mod), ("quotient", numericBinop quot), ("=", numBoolBinop (==)), ("<", numBoolBinop (<)),
  (">", numBoolBinop (>)), ("/=", numBoolBinop (/=)), (">=", numBoolBinop (>=)), ("<=", numBoolBinop (<=)),
  ("&&", boolBoolBinop (&&)), ("||", boolBoolBinop (||)), ("string=?", strBoolBinop (==)), ("string<?", strBoolBinop (<)),
  ("string>?", strBoolBinop (>)), ("string<=?", strBoolBinop (<=)), ("string>=?", strBoolBinop (>=)), ("remainder", numericBinop rem)]

apply :: String -> [LispVal] -> ThrowsError LispVal
apply func args = 
  maybe(throwError $ NotFunction "Unrecognized primtive function arg" func) ($ args) $ lookup func primitives
...
car badArgList = throwError $ NumArg 1 badArgList
...
cdr badArgList = throwError $ NumArg 1 badArgList
...
readExpr :: String -> ThrowsError LispVal; readExpr input = case parse parseExpr "lisp" input of
  Left err -> throwError $ Parser err
  Right val -> return val

main = do
    let args = "(if (= 2 3) (+ 2 3 (- 5 1)) \"unequal\")"
    evaled <- return $ liftM show $ readExpr args >>= eval
    putStrLn $ extractValue $ trapError evaled

{-
runghc src/haskell/src/Main.hs
-}